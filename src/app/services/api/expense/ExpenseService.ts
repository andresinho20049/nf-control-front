...
const findPaginated = async (page = 1, size = 5, year: string): Promise<IPageableData<IExpenseData> | Error> => {

    try {
        const { data } = await ApiForm.get(`/expense/paginated?page=${page - 1}&size=${size}&year=${year}`);
